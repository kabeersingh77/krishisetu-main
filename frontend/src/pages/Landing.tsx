    ...
    <div className="flex flex-col min-h-screen bg-background text-foreground">
      ...
      <section className="relative px-6 py-24 md:py-32 overflow-hidden border-b bg-gradient-to-b from-primary/5 to-background">
        <div className="container mx-auto max-w-5xl text-center space-y-8">
          <span className="inline-block rounded-full border bg-primary/10 text-primary hover:bg-primary/20 mb-4 px-3 py-1 text-sm border-primary/20 font-semibold">
            Agriflow Direct Sourcing Architecture
          </span>
          <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight">
            Fairer prices begin with a{" "}
            <span className="text-emerald-600">shorter supply chain.</span>
          </h1>
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
            Agriflow connects farmers and FPOs directly with consumers and bulk
            buyers while using intelligent pricing, demand forecasting, and
            logistics optimization to make agricultural commerce more efficient.
          </p>
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 pt-6">
            <Link to="/marketplace">
              <Button
                size="lg"
                className="w-full sm:w-auto h-12 px-8 text-lg bg-emerald-600 hover:bg-emerald-700 text-white"
              >
                Explore Marketplace
              </Button>
            </Link>
        ...
        <div className="container mx-auto max-w-6xl space-y-12">
          <div className="text-center space-y-4">
            ...
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
              Too many intermediaries mean farmers earn less while consumers pay
              more.
            </p>
          </div>
            ...
            <Card className="border-red-200 bg-red-50/50 dark:bg-red-950/10 dark:border-red-900">
              <CardContent className="p-8 space-y-6">
                <h3 className="text-xl font-bold text-red-600 dark:text-red-400">
                  Traditional Chain (6 Intermediary Tiers)
                </h3>
              ...
              <CardContent className="p-8 space-y-6">
                ...
                <div className="pt-6 mt-6 border-t text-center">
                  <p className="font-semibold text-lg">
                    4 Intermediaries Removed.
                  </p>
                  <p className="text-muted-foreground">
                    Farmers earn 75% more. Consumers save 30%.
                  </p>
                </div>
              </CardContent>
            </Card>
        ...
        <div className="container mx-auto max-w-6xl">
          <div className="text-center space-y-4 mb-16">
            ...
            <p className="text-muted-foreground text-lg">
              Technology that empowers agricultural commerce.
            </p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            <FeatureCard
              ...
              desc="Connect directly with farmers and FPOs for transparent sourcing."
            />
            <FeatureCard
              icon={TrendingUp}
              title="AI Price Engine"
              desc="Data-driven pricing algorithms ensure fair transactions for everyone."
            />
            <FeatureCard
              icon={LineChart}
              title="Demand Forecasting"
              desc="Predict market demand to reduce post-harvest waste."
            />
            <FeatureCard
              icon={Route}
              title="Smart Logistics"
              desc="Optimized delivery routes for faster, cost-effective transport."
            />
            <FeatureCard
              icon={Building2}
              title="Bulk Procurement"
              desc="Streamlined purchasing flows tailored for institutional buyers."
            />
            <FeatureCard
              icon={ShieldCheck}
              title="Quality Assurance"
              desc="Standardized grading systems build trust in every transaction."
            />
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto max-w-6xl text-center space-y-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            <Metric
              ...
              value={impactMetrics.farmersConnected}
            />
            <Metric
              title="Produce Traded"
              value={impactMetrics.produceTraded}
            />
            <Metric
              title="Orders Fulfilled"
              value={impactMetrics.ordersFulfilled}
            />
            <Metric
              title="Farmer Income Growth"
              value={impactMetrics.avgFarmerIncrease}
            />
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto max-w-3xl space-y-12">
          ...
          <Accordion type="single" collapsible className="w-full">
            <AccordionItem value="item-1">
              ...
              <AccordionContent>
                Our AI analyzes historical market data, real-time demand, local
                weather conditions, and seasonal trends to suggest a fair price
                range. This prevents extreme price manipulation by middlemen and
                ensures farmers get fair compensation.
              </AccordionContent>
            </AccordionItem>
            <AccordionItem value="item-2">
              <AccordionTrigger>Who handles the logistics?</AccordionTrigger>
              <AccordionContent>
                Agriflow partners with local logistics providers and delivery
                agents. Our Route Optimizer ensures that pick-ups and drop-offs
                are clustered efficiently to minimize transit time and costs.
              </AccordionContent>
            </AccordionItem>
            <AccordionItem value="item-3">
              <AccordionTrigger>Is it only for bulk buyers?</AccordionTrigger>
              <AccordionContent>
                No. Agriflow serves both individual consumers buying weekly
                groceries and bulk buyers (restaurants, institutions) procuring
                tons of produce. The platform automatically routes these
                differently to optimize the supply chain.
              </AccordionContent>
            </AccordionItem>
          </Accordion>
        </div>
      ...
      <section className="py-24 px-6 bg-muted text-center border-t">
        <div className="container mx-auto max-w-2xl space-y-8">
          <h2 className="text-4xl font-bold">Join Agriflow today</h2>
          <p className="text-xl text-muted-foreground">
            Experience the future of agricultural commerce.
          </p>
          <Link to="/register">
            <Button
              size="lg"
...
function ChainStep({
  ...
  return (
    <div
      className={`flex justify-between items-center p-3 rounded-md border ${highlight ? "bg-red-100 dark:bg-red-900 border-red-300" : "bg-background border-border"}`}
    >
      <span className="font-medium">{name}</span>
      <span
        className={`font-semibold ${highlight ? "text-red-700 dark:text-red-300" : ""}`}
      >
        {price}
      </span>
    </div>
