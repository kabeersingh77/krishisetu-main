  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      res.json({
        ...
        avgRealization:
          totalQuantitySold > 0
            ? Math.round((totalRevenue / totalQuantitySold) * 100) / 100
            : 0,
        revenueChart,
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const consumers = await prisma.user.count({
        where: { role: "CONSUMER" },
      });
      const bulkBuyers = await prisma.user.count({
        where: { role: "BULK_BUYER" },
      });
      ...
      orders.forEach((o) => {
        o.items.forEach((i) => {
          farmerEarnings += i.price * i.quantity;
        });
      });
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      res.json({
        ...
        revenue: Math.round(revenue * 100) / 100,
        avgFarmerRealization:
          orderSet.size > 0
            ? Math.round((revenue / orderSet.size) * 100) / 100
            : 0,
      });
    } catch (error) {
...
router.get("/impact", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const avgFarmerRealization =
      totalConsumerSpent > 0
        ? (totalFarmerEarnings / totalConsumerSpent) * 100
        : 85;

    // Traditional model comparison (simulate)
    // In traditional chain, farmer gets ~30-40% of consumer price
    // In Agriflow, farmer gets ~80-90% (platform fee ~5-10%, logistics ~5-10%)
    const traditionalFarmerShare = 35; // percentage
    const AgriflowFarmerShare = Math.round(avgFarmerRealization);
    const consumerSavings = Math.round(
      (1 - AgriflowFarmerShare / 100 / (traditionalFarmerShare / 100)) * -100,
    );
    ...
    const avgFulfillmentDist =
      completedDeliveries.length > 0
        ? completedDeliveries.reduce((sum, d) => sum + (d.distance || 0), 0) /
          completedDeliveries.length
        : 0;

    const farmers = await prisma.user.count({ where: { role: "FARMER" } });
    const fpoMembers = await prisma.fPO.aggregate({
    ...
    res.json({
      ...
      avgFarmerRealization: AgriflowFarmerShare,
      traditionalFarmerShare,
      farmerRealizationImprovement:
        AgriflowFarmerShare - traditionalFarmerShare,
      ...
      logisticsEfficiency:
        completedDeliveries.length > 0
          ...
          : 0,
      // Traditional vs Agriflow breakdown (per ₹100 of consumer price)
      traditional: {
        farmerShare: 35,
        localTrader: 10,
        wholesaler: 15,
      ...
      Agriflow: {
        farmerShare: AgriflowFarmerShare,
        platformFee: 5,
        logistics: 100 - AgriflowFarmerShare - 5,
      },
      disclaimer:
        "Prototype simulation based on platform transaction data. Traditional supply chain percentages are illustrative estimates.",
    });
  } catch (error: any) {
